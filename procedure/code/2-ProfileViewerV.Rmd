...
ggplot(LongProf) +
       ...
       scale_color_gradient(low = "gray", high = "red") +
       theme_light() +
       labs(x = "Distance along the river profile (m)", y = "Elevation (masl)", title = "Longitudinal Profile of the Extracted Reach", color = "Slope %")
```
...
DpthBf_Max <- 0.9657 # Insert the maximum bankfull depth from your point/reach of interest here, replacing the 8 left of the hash #
...
ggplot(CrossProf) +
       aes(x = Distance, y = Elevation)+ # , color = Elevation) +
       geom_point(color = "darkblue") + # Point chosen here to allow easier determining of the width (points are horizontally spaced by 1 meter)
       geom_hline(yintercept = MaxBFx2Elev) +
       xlim(120,230) + # Sets the x-axis limits. Adjust this value as you zoom in to the correct area.
       ylim(1169,1174) + # Sets the y-axis limits. Adjust this value as you zoom in to the correct area.
       # scale_color_gradient(low = "green", high = "brown") +
       annotate("text", x=207, y=1169.7, label= "*channel location") + 
       annotate("text",x=160,y=1172.05, label="flood-prone elevation")+
       labs(x = "Distance along the transect (m)", y = "Elevation (masl)", title = "Cross-Sectional Profile of the Flood-Prone Part of the Valley")
